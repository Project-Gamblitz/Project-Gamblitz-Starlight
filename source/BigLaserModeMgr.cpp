...
static void (*sOrigWeaponOnActivate)(void *, bool) = NULL;

static void weaponOnActivateHook(void *weapon, bool arg) {
	sOrigWeaponOnActivate(weapon, arg);

	// Neither asset is named "OnActivate", so the XLink auto-play does nothing.
	// Manually play the correct asset based on mode.
	if (!BigLaserModeMgr::sInstance) return;
	...
	for (int i = 0; i < sWeaponTrackCount; i++) {
		...
		if (xlink) {
			...
			const char *assetName = (sWeaponTracks[i].activeMode == cPrincessCannon)
				? "OnActivatePC" : "OnActivateKW";
			xlink->searchAndPlayWrap(assetName, false, &handle);
		}
		break;
	}
}
...
void BigLaserModeMgr::initModelHook() {
	...
	// Hook PlayerWeaponBigLaser vtable entries.
	Cmn::ActorVtable *pwblVtable = (Cmn::ActorVtable *)ProcessMemory::MainAddr(0x29FA4E0);

	// onActivate hook: play PC sound when in Princess Cannon mode
	sOrigWeaponOnActivate = (void (*)(void *, bool))pwblVtable->onActivate;
	pwblVtable->onActivate = (u64)weaponOnActivateHook;
